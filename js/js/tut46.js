/*JavaScript Regular Expressions - Basic Functions | JavaScript Tutorial In Hindi #46
...
let reg = /harry/;  // pata hai hasi aarahi par ye hi hai regular expression
//  reg = /harry/g;  // g means global 
// reg = /harry/i; // i means case insensitive
// console.log(reg);
// console.log(reg.source); // code ke ander ka masala ki code me likha kya hai jaise yaha pe harry

let s = "This is great code with harry and also harry bhai";
//function to match expression
// 1. exec() - This function will return an array for match or null for no match
let result = reg.exec(s /*jo string ka nam hai vo jaise yaha pr s hai */);

result = reg.exec(s); // console.log(result); // jitni bar tujhe run karvana hai utni bar isse replicate kar matlab sentence me 2 bar same word hai or tujhe dono chahiye to issi function ko 2 bar replicate kar 
// result = reg.exec(s); // agar 3sara word nahi hai same to null aayega
...
let result5 = s.replace(reg, 'SHUBHAM'); // replace reg to given value
// console.log(result5);

// khud se banaya hai maze me 
if (result2 == true) {
    console.log(`${reg.source} founded in the string at index ${result.index}`)
    console.log(`Replacing value \n${result5}`)

}
else {
    // console.log(`${reg.source} not founded in the string`)
}
